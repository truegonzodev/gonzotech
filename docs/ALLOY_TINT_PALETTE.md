...
`gonzotech:alloy_tint` и показывается в tooltip. При рендере специальный
client-only renderer использует этот неизменный `C_mix` в первых трёх шагах
композиции и переносит его hue/saturation при сохранении Rec.709 светлоты
серого PNG. Затем он добавляет детерминированную 70/30 мозаику из **этих же
исходных palette RGB** всех ингредиентов, распределённую по MU. Никакой
render-only HSV/value-нормализации в data component нет: `alloy_tint` остаётся
точным серверным значением.
