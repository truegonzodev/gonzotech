...
public final class ResourceBuffer {
     ...
     */
    public int receive(long want, boolean simulate) {
        if (want <= 0) return 0;
        // want может прийти как long (единый Sink API), но mB-ёмкости малы (int).
        int w = want > Integer.MAX_VALUE ? Integer.MAX_VALUE : (int) want;
        int accepted = Math.min(w, space());
     ...
     */
    public int extract(long want, boolean simulate) {
        if (want <= 0) return 0;
        int w = want > Integer.MAX_VALUE ? Integer.MAX_VALUE : (int) want;
        int given = Math.min(w, amount);
