...
public final class MachineDefs {
    ...
    // ═══════════════════════════ ЛОГИСТИКА: ТРУБЫ/СЕТИ ═══════════════════════════
    // Скелет: числа предварительные, финальный баланс автор задаст позже.
    // Сеть тикает целиком: за тик тянет из источников в транзитный буфер и сливает
    // из буфера в приёмники, каждая фаза ограничена «throughput». Буфер — крошечный
    // транзит (сглаживает неравные концы), НЕ хранилище.

    /** Провод (GTU): макс. передача за тик через один контур. */
    public static final int WIRE_THROUGHPUT = 256;
    /** Провод (GTU): ёмкость транзитного буфера контура. */
    public static final int WIRE_BUFFER = 1_024;

    /** Теплотруба (GTH): макс. передача за тик через один контур. */
    public static final int HEAT_THROUGHPUT = 256;
    /** Теплотруба (GTH): ёмкость транзитного буфера контура. */
    public static final int HEAT_BUFFER = 1_024;
