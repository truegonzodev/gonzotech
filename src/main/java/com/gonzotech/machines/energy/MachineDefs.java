...
public final class MachineDefs {
    ...
    // Отдельных throughput/buffer-констант у труб НЕТ.
    ...
    /** Максимум GTU в помпе. */
    public static final int PUMP_GTU_CAPACITY = 1_200;
    /** Максимум воды в помпе, mB. */
    public static final int PUMP_WATER_CAPACITY = 24_000;

    /** Макс. приём GTU за тик (из провода). */
    public static final int PUMP_GTU_INTAKE = 64;

    /**
     * Пассивный расход помпы, milli-GTU за тик (2.8 GTU/t). Пока есть чем платить —
     * помпа «запитана» и работает; иначе стоит. Дробь считается через аккумулятор,
     * как у электропечи.
     */
