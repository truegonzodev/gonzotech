...
public final class AirFilterMenu extends BaseMachineMenu {
    ...
    private AirFilterMenu(int id, Inventory inventory, Container container, ContainerData data, BlockPos pos) {
        ...
        for (int slot = 0; slot < 3; slot++) {
            addSlot(new Slot(container, slot, 62 + slot * 18, 124) {
                @Override public boolean mayPlace(ItemStack stack) { return AirFilterBlockEntity.isSupply(stack); }
            });
        }
        // Author's reference: canvas (136,145), step 18; hotbar (136,212).
        for (int row = 0; row < 3; row++) {
            for (int column = 0; column < 9; column++) {
                addSlot(new Slot(inventory, column + row * 9 + 9, 8 + column * 18, 17 + row * 18));
            }
        }
        for (int column = 0; column < 9; column++) addSlot(new Slot(inventory, column, 8 + column * 18, 84));
    }

    public int gtu() { return (data.get(0) & 0xffff) | ((data.get(1) & 0xffff) << 16); }
    public int coalSeconds() { return data.get(2); }
    public int catalystSeconds() { return data.get(3); }
