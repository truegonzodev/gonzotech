...
public class MarsPillarFeature extends Feature<NoneFeatureConfiguration> {
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
        ...
        for (int i = 0; i < pillars; i++) {
            ...
            // Центр столба со сдвигом от центра чанка (кластеризация, разброс * 12).
            int cx = chunkMinX + 8 + (int) Math.round((random.nextDouble() * 2 - 1) * 12);
            int cz = chunkMinZ + 8 + (int) Math.round((random.nextDouble() * 2 - 1) * 12);
